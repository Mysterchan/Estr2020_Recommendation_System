...
def SGD(train,test,N,M,Level,eta,K,lambda_1,lambda_2,lambda_3,Step):
    ...
    for ste in range(Step):
        ...
        for data in train:
            ...
            Yu=Y[u]
            Vi=V[i]
            total_Y += sum(np.square(Yu))
            total_V += sum(np.square(Vi))
        ...
        print(f"total_y: {total_Y}, {total_V}")
        rms=RMSE(U,V,test,Level)
        rmse.append(rms)
        print(rmse)
    return rmse,U,V
...
def Load_data(filedir,ratio):
    ...
    for line in f.readlines():
        ...
        if int(i) not in item_set:
            ...
            i_idx+=1
        if float(r) not in rating_set:
            rating_set[float((float(r)-0.5)/4.5)]=r_idx
            r_idx+=1
        data.append([user_set[int(u)],item_set[int(i)],float((float(r)-0.5)/4.5)])
    f.close()
    N=u_idx
    M=i_idx
    Level=r_idx
